...
export default function AdvantageSection({ setView }: AdvantageSectionProps) {
  ...
  return (
    <section id="advantage" className="py-16 px-4 md:px-8 max-w-7xl mx-auto w-full text-left">
      ...
      >
        <h2 className="text-[12px] font-bold text-[#0F0F8B] uppercase tracking-widest mb-2 font-mono">The Engraced Logistics Standard</h2>
      ...
      >
        ...
        >
          <div className="absolute -right-10 -bottom-10 w-52 h-52 bg-[#0F0F8B]/10 rounded-full blur-3xl group-hover:bg-[#0F0F8B]/20 transition-all duration-500"></div>
          ...
          <div className="relative z-10 w-[58%] md:w-3/5 pr-2 md:pr-4">
            ...
            <h4 className="text-[12px] xs:text-xs md:text-sm font-bold text-zinc-950 uppercase tracking-wide mb-1 md:mb-2">Professional Chauffeur & Logistics Support</h4>
            <p className="text-[10px] xs:text-[11.5px] md:text-xs lg:text-sm text-zinc-500 leading-normal md:leading-relaxed">
        ...
        >
          <div className="w-10 h-10 bg-zinc-50 rounded-xl flex items-center justify-center text-zinc-950 mb-6 group-hover:bg-[#0F0F8B] group-hover:text-white transition-colors">
            <Clock size={18} className="text-[#0F0F8B]" />
          </div>
          <div>
        ...
        >
          <div className="w-10 h-10 bg-zinc-50 rounded-xl flex items-center justify-center text-zinc-950 mb-6 group-hover:bg-[#0F0F8B] group-hover:text-white transition-colors">
            <Zap size={18} className="text-[#0F0F8B]" />
          </div>
          <div>
        ...
        <motion.div 
          variants={itemVariants}
          className="md:col-span-2 bg-gradient-to-br from-[#0F0F8B] to-[#080870] rounded-[2rem] p-5 sm:p-7 md:p-8 lg:p-10 relative overflow-hidden flex items-center justify-between active:scale-[0.99] transition-transform duration-300 h-[170px] xs:h-[190px] md:h-auto"
        >
          ...
          <div className="relative z-10 w-[58%] md:w-3/5 pr-2 md:pr-4 text-left">
            <h4 className="text-[10px] xs:text-[11.5px] font-bold text-[#0000FD] uppercase tracking-widest mb-1 font-mono">Utility Fleet</h4>
            <h4 className="text-[12px] xs:text-xs md:text-sm font-bold text-white uppercase tracking-wide mb-1.5 md:mb-2.5">Toyota Hilux Utility Fleet</h4>
            <p className="text-[10px] xs:text-[11.5px] md:text-xs text-zinc-400 leading-normal md:leading-relaxed max-w-sm">
          ...
          <img 
            ...
            className="absolute right-0 top-6 md:top-10 h-full w-[38%] object-cover rounded-tl-[1.5rem] shadow-2xl border-l border-t border-[#0F0F8B]/30" 
          />

          <button 
            onClick={() => setView('booking')}
            className="hidden md:flex absolute right-6 top-1/2 -translate-y-1/2 z-20 w-12 h-12 bg-[#0000FD] hover:bg-white text-[#080870] hover:text-[#080870] rounded-full items-center justify-center cursor-pointer active:scale-90 transition-all shadow-xl"
          >
