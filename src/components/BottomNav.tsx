...
export default function BottomNav({ currentView, setView }: BottomNavProps) {
  ...
  return (
    <div className="lg:hidden fixed bottom-6 left-1/2 -translate-x-1/2 w-[92%] max-w-sm bg-zinc-950/95 backdrop-blur-xl border border-zinc-800 rounded-2xl p-2.5 flex justify-around items-center z-50 shadow-2xl">
      {menu.map((item) => {
        ...
        return (
          <button 
            ...
            className={`p-2 flex flex-col items-center gap-1 active:scale-90 transition-transform cursor-pointer focus:outline-none ${
              isActive ? 'text-[#0152FA]' : 'text-zinc-400 hover:text-white'
            }`}
          >
            <Icon size={18} />
