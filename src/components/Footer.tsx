  ...
  return (
    <footer className="bg-gradient-to-br from-[#0F0F8B] to-[#080870] text-white pt-24 pb-28 md:pb-24 px-6 md:px-12 relative overflow-hidden border-t border-[#0F0F8B]/20 text-left">
      ...
      <div className="max-w-7xl mx-auto relative z-10">
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-10 lg:gap-12 mb-20">
          ...
          <div className="flex flex-col gap-4 text-left">
            ...
            <div className="flex flex-col gap-2.5">
              <button onClick={() => setView('home')} className="text-[15px] text-white hover:text-[#0000FD] cursor-pointer text-left transition-colors font-sans hover:translate-x-1 duration-200">Overview Hub</button>
              <button onClick={() => setView('about')} className="text-[15px] text-white hover:text-[#0000FD] cursor-pointer text-left transition-colors font-sans hover:translate-x-1 duration-200">About Us</button>
              <button onClick={() => setView('booking')} className="text-[15px] text-white hover:text-[#0000FD] cursor-pointer text-left transition-colors font-sans hover:translate-x-1 duration-200">Booking Desk</button>
            </div>
          </div>
          ...
          <div className="flex flex-col gap-4 text-left">
            ...
            <div className="flex flex-col gap-2.5">
              <button onClick={() => setView('terms')} className="text-[15px] text-white hover:text-[#0000FD] cursor-pointer text-left transition-colors font-sans hover:translate-x-1 duration-200">Terms of Service</button>
              <button onClick={() => setView('privacy')} className="text-[15px] text-white hover:text-[#0000FD] cursor-pointer text-left transition-colors font-sans hover:translate-x-1 duration-200">Privacy Policy</button>
            </div>
          </div>
          ...
          <div className="flex flex-col gap-4 text-left">
            ...
            <form onSubmit={(e) => e.preventDefault()} className="flex gap-2 mt-1">
              <input 
                ...
                className="w-full bg-[#0A0A78] border border-[#0F0F8B]/30 rounded-xl px-3.5 py-2 text-[16.5px] text-white placeholder-zinc-500 focus:outline-none focus:border-[#0F0F8B]/50 transition-colors font-sans"
              />
              <button 
                type="submit" 
                className="bg-[#0F0F8B] hover:bg-[#0A0A78] text-white font-bold uppercase tracking-widest text-[15.5px] px-3.5 py-2 rounded-xl cursor-pointer active:scale-95 transition-all shrink-0"
              >
                Sub
              </button>
            </form>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-6 pt-10 pb-8 border-t border-[#0F0F8B]/20 text-left text-white text-[16px] font-mono uppercase tracking-wider relative z-20">
        ...
        <div className="flex flex-col md:flex-row items-center justify-between pt-8 border-t border-[#0F0F8B]/20 gap-6 relative z-20">
          ...
          <div className="flex gap-3">
             <div onClick={() => setView('home')} className="w-8 h-8 rounded-full bg-[#0A0A78] border border-[#0F0F8B]/30 flex items-center justify-center hover:bg-[#0F0F8B] hover:text-white hover:border-[#0000FD]/30 transition-all duration-300 cursor-pointer text-white shadow-sm" title="Overview Hub"><Globe size={13}/></div>
             <div onClick={() => setView('about')} className="w-8 h-8 rounded-full bg-[#0A0A78] border border-[#0F0F8B]/30 flex items-center justify-center hover:bg-[#0F0F8B] hover:text-white hover:border-[#0000FD]/30 transition-all duration-300 cursor-pointer text-white shadow-sm" title="About Us">
                <span className="font-mono text-[15px] font-bold">US</span>
             </div>
             <div onClick={() => setView('terms')} className="w-8 h-8 rounded-full bg-[#0A0A78] border border-[#0F0F8B]/30 flex items-center justify-center hover:bg-[#0F0F8B] hover:text-white hover:border-[#0000FD]/30 transition-all duration-300 cursor-pointer text-white shadow-sm" title="Terms"><span className="font-mono text-[15px] font-bold">TS</span></div>
          </div>
        </div>
      </div>
